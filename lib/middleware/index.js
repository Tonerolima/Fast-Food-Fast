...
middleware.createUser = (req, res, next) => {
  ...
  if (!newUser) { return res.status(400).send({ status: false, message: 'User already exists' }); }
  return next();
};
...
middleware.authenticate = (req, res, next) => {
  ...
  if (!user) { return res.status(404).send({ status: false, message: 'Incorrect username' }); }
